...
def todo_detail(request, todo_pk):
    todo = Todo.objects.annotate(num_of_recommends=Count('recommend')).get(pk=todo_pk)
...
def recommend_create(request, todo_pk):
    todo = Todo.get_object_or_404(pk=todo_pk)
    # todo = Todo.objects.get(pk=todo_pk)
    if request.method == 'POST':
        serializer = RecommendSerializer(data=request.POST)
        if serializer.is_valid(raise_exception=True):
